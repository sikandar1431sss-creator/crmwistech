 ...
 * @return string
 */
function staff_email_signature_image($id = false, $classes = array('staff-profile-image'), $type = '', $img_attrs = array())
{
    $url = base_url('assets/images/user-placeholder.jpg');

    $CI =& get_instance();
    $CI->db->select('email_signature_image,firstname,lastname');
    $CI->db->where('staffid', $id);
    $result = $CI->db->get('tblstaff')->row();
    ...
    $type = '';
    if ($result && $result->email_signature_image !== null) {

        $profileImagePath = 'uploads/staff_profile_images/' . $id . '/' . $result->email_signature_image;
        if (file_exists($profileImagePath)) {
            $profile_image = '<img ' . $_attributes . ' src="' . base_url($profileImagePath) . '" class="' . implode(' ', $classes) . '" alt="' . $result->firstname . ' ' . $result->lastname . '" />';
        } else {
            return $blankImageFormatted;
...
if (!function_exists('get_table_items_and_taxes')) {
    ...
    function get_table_items_and_taxes($items, $type, $admin_preview = false)
    {
        ...
        foreach ($items as $item) {
            $item_rate = $item['rate'];
            if($CI->session->userdata('discount_type_fixed') == "1") {
                $discount_proportion= $CI->session->userdata('discount_proportion');
                $discount  = ($item['rate'] * $discount_proportion)/100;
                $item_rate = $item['rate'] - ($discount );
            }
            $_item = '';
            $tr_attrs = '';
            $td_first_sortable = '';
            ...
            $long_description= str_replace($tags, "<br>", $long_description);
            $description = $item['description'];
            $description= str_replace($tags, "<br>", $description);

            $_item .= '<td class="description" align="left;"><span style="font-size:' . (isset($font_size) ? $font_size + 4 : '') . 'px;"><strong>' . strip_tags($description) . '</strong></span><br><span style="color:#424242;">' . strip_tags($long_description,'<a></a><ul></ul><li></li>') . '</span>';

          //  $_item .= '<td class="description" align="left;"><span style="font-size:' . (isset($font_size) ? $font_size + 4 : '') . 'px;"><strong>' . $item['description'] . '</strong></span><span style="color:#424242;">' . $item['long_description'] . '</span>';

            if($item["is_renew_able"]){
                $_item .= '<br/><span style="color:#424242;"> Starting From ' . date_format_dmy($item['start_date']) . ' to ' . date_format_dmy($item["end_date"]).'</span>';
            }
            ...
            if (count($item_taxes) > 0) {
                foreach ($item_taxes as $tax) {
                    ...
                    if ($tax_not_calc == true) {
                        ...
                        array_push($result['taxes'][$tax['taxname']]['total'], (($item['qty'] * $item_rate) / 100 * $tax['taxrate']));
      ...
      /*  $CI->session->unset_userdata('discount_proportion');
        $CI->session->unset_userdata('discount_type_fixed');*/
