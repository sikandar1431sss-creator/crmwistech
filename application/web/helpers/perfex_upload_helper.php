...
/**
 * Check for staff Eamil Signature image
 * @return boolean
 */
function handle_staff_email_signature_image_upload($staff_id = '')
{
    if (!is_numeric($staff_id)) {
        $staff_id = get_staff_user_id();
    }
    if (isset($_FILES['email_signature_image']['name']) && $_FILES['email_signature_image']['name'] != '') {
        do_action('before_upload_staff_profile_image');
        $path = get_upload_path_by_type('staff') . $staff_id;

        // Get the temp file path
        $tmpFilePath = $_FILES['email_signature_image']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            // Getting file extension
            $path_parts         = pathinfo($_FILES["email_signature_image"]["name"]);
            $extension          = $path_parts['extension'];
            $extension = strtolower($extension);
            $allowed_extensions = array(
                'jpg',
            ...
            _maybe_create_upload_path($path);
            $filename    = unique_filename($path, $_FILES["email_signature_image"]["name"]);
            $newFilePath = $path . '/' . $filename;
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                $CI =& get_instance();
                ...
                $CI->db->where('staffid', $staff_id);
                $CI->db->update('tblstaff', array(
                    'email_signature_image' => $filename
                ));
                // Remove original image
                // unlink($newFilePath);
...
echo '<pre>';
print_r($_FILES["email_signature_image"]["error"]);
die;
            }
        }
    }
