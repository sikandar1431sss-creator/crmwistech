...
$pdf->Ln(3);
$info_right_column .= '<b>No #: </b>' . format_receipt_number($receipts->receipt_id);
...
$pdf_bank_name = function_exists('get_receipt_bank_name') ? get_receipt_bank_name($receipts) : $receipts->receipt_bank;
...
if (count($invoices) > 0) {
    ...
    foreach ($invoices as $item) {
        ...
        $tblhtml .= '    <td align="center">' . $item->total_amount . '</td>';
        $tblhtml .= '    <td align="center">' . $item->amount . '</td>';
        $tblhtml .= '</tr>';
    }
    ...
    $tblhtml .= '    <td align="center">' . format_money($total, ' AED ') . '</td>';
    $tblhtml .= '</tr>';
