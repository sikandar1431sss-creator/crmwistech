                    ...
                    <ul class="nav nav-tabs nav-tabs-horizontal mbot15" role="tablist">
                        ...
                        <?php if (count($invoice->payments) > 0) { ?>
                            <li role="presentation">
                                <a href="#invoice_payments_received" aria-controler="invoice_payments_received"
                     ...
                     class="invoice-send-to-client btn-with-tooltip btn btn-default<?php if ($invoice->status == 5) {
                                ...
                                if ((get_option('delete_only_on_last_invoice') == 1 && is_last_invoice($invoice->id)) || (get_option('delete_only_on_last_invoice') == 0)) { ?>
                                    <?php if (has_permission('invoices', '', 'delete')) { ?>
                                        <li data-toggle="tooltip"
                                            data-title="<?php echo _l('delete_invoice_tooltip'); ?>">
                        ...
                        <?php if (has_permission('payments', '', 'create') && abs($invoice->total) > 0) { ?>
                            <a href="#" onclick="record_payment(<?php echo $invoice->id; ?>); return false;"
                               class="mleft10 pull-right btn btn-success<?php if ($invoice->status == 2 || $invoice->status == 5) {
                                   echo ' disabled';
                               } ?>">
                ...
                <div role="tabpanel" class="tab-pane active" id="tab_invoice">
                    <?php if ($invoice->status == 5 && $invoice->recurring > 0) { ?>
                        <div class="alert alert-info">
                            ...
                            want to stop this recurring invoice you should update the invoice recurring field to
                            <b>No</b>.
                        </div>
                    <?php } ?>
                                        ...
                                        <td><?php echo format_money($credit['amount'], $invoice->symbol) ?>
                                            <?php if (has_permission('credit_notes', '', 'delete')) { ?>
                                                <a href="<?php echo admin_url('credit_notes/delete_invoice_applied_credit/' . $credit['id'] . '/' . $credit['credit_id'] . '/' . $invoice->id); ?>"
                                                   class="pull-right text-danger _delete"><i
