...
class Clients_model extends CRM_Model
{
    ...
    public function get_statement($customer_id, $from, $to)
    {
        ...
        tblinvoices.clientnote as invoice_note,
        ...
        $sql2 = 'SELECT
        tblestimates.id as pinvoice_id,
        tblestimates.clientnote as pinvoice_note,
        tblestimates.status as pinvoice_status,
        tblestimates.date as date,
        concat(tblestimates.date, \' \', RIGHT(tblestimates.datecreated,LOCATE(\' \',tblestimates.datecreated) - 3)) as tmp_date,
        tblestimates.total as pinvoice_amount
        FROM tblestimates WHERE tblestimates.clientid =' . $customer_id;
        ...
        /*$proforma_invoices = $this->db->query($sql2 . '
            AND status != 6
            ORDER By date DESC')->result_array();*/
        $proforma_invoices = [];
        ...
        /*  $sqlDatePayments = str_replace('date', 'tblinvoicepaymentrecords.date', $sqlDate);
          ...
          tblreciepts.receipt_id as receipt_id,
          tblreciepts.receipt_num as receipt_num,
          ...
          SUM(tblinvoicepaymentrecords.amount) as payment_total_receipt
          FROM tblinvoicepaymentrecords
          JOIN tblinvoices ON tblinvoices.id = tblinvoicepaymentrecords.invoiceid
          JOIN tblreciepts ON tblreciepts.receipt_id = tblinvoicepaymentrecords.receipt_id
          WHERE ' . $sqlDatePayments . ' AND tblinvoices.clientid = ' . $customer_id . '
          GROUP BY tblinvoicepaymentrecords.receipt_id
        ...
        $merged = array_merge($merged, $proforma_invoices);
        usort($merged, function ($a, $b) {
            // fakde date select sorting
            return strtotime($a['tmp_date']) - strtotime($b['tmp_date']);
        });
        ...
        // Proforma Invoiced amount during the period
        $result['pinvoiced_amount'] = $this->db->query('SELECT
        SUM(tblestimates.total) as pinvoiced_amount
        FROM tblestimates
        WHERE clientid = ' . $customer_id . '
        AND ' . $sqlDate . ' AND status != 5 and status != 6')
            ->row()->pinvoiced_amount;

        if ($result['pinvoiced_amount'] === null) {
            $result['pinvoiced_amount'] = 0;
        }
      ...
      /*  $result['amount_paid'] = $this->db->query('SELECT
        ...
        $result['amount_paid'] = $this->db->query('SELECT
        SUM(tblreciepts.receipt_amount) as amount_paid
        FROM tblreciepts
        WHERE ' . $sqlDateReceipts . ' AND tblreciepts.receipt_client_id = ' . $customer_id)
            ->row()->amount_paid;
       ...
       /* $result['beginning_balance'] = $this->db->query('
        ...
        $result['beginning_balance'] = $this->db->query('
            SELECT (CAST(SUM(tblinvoices.total) - (SELECT COALESCE(SUM(tblreciepts.receipt_amount),0)
            FROM tblreciepts
            WHERE tblreciepts.receipt_date < "' . $from . '"
            AND tblreciepts.receipt_client_id=' . $customer_id . ') as SIGNED))
            as beginning_balance FROM tblinvoices
            WHERE date < "' . $from . '"
            AND clientid = ' . $customer_id . '
            AND status != 6
        ...
        $result['invoiced_amount']=  $result['invoiced_amount'] + $result['pinvoiced_amount'];
    ...
    public function updateZohoId($id, $zoho_id){

        $this->db->where('userid', $id);
        $this->db->update('tblclients', array('zoho_id' => $zoho_id));
        return true;
