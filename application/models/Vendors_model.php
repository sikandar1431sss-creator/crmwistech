...
{
    private $vendor_columns;

    public function __construct()
    {
        parent::__construct();

        $this->vendor_columns = do_action('vendor_columns', ['firstname', 'firstname', 'lastname', 'company_name', 'contact_display', 'contact_email', 'contact_mobile', 'contact_work', 'contact_web', 'tax_treatemnt', 'tax_number', 'payment_terms', 'note']);
    ...
    {
        $this->db->select(implode(',', prefixed_table_fields_array('tlbvendors')));
        $this->db->where($where);

        if (is_numeric($id)) {
            $this->db->where('tlbvendors.id', $id);
            $vendors = $this->db->get('tlbvendors')->row();
            return $vendors;
        }

        $this->db->order_by('company_name', 'asc');
        return $this->db->get('tlbvendors')->result_array();
    }
