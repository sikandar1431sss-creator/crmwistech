...
class Currencies_model extends CRM_Model
{
    ...
    public function get($id = false)
    {
        if (is_numeric($id)) {
            ...
            return $this->normalize_currency_symbol_row($this->db->get('tblcurrencies')->row());
        }
        ...
        foreach ($currencies as $index => $currency) {
            $currencies[$index] = $this->normalize_currency_symbol_row($currency);
        }
    ...
    public function get_currency_symbol($id)
    {
        ...
        if ($currencies) {
            foreach ($currencies as $currency) {
                if ($id == $currency['id']) {
                    return $this->safe_currency_symbol($currency['name'], $currency['symbol']);
                }
            }
        }

        $this->db->select('name, symbol');
        $this->db->from('tblcurrencies');
        $this->db->where('id', $id);

        $currency = $this->db->get()->row();

        return $currency ? $this->safe_currency_symbol($currency->name, $currency->symbol) : '';
    }

    private function normalize_currency_symbol_row($currency)
    {
        if (is_array($currency)) {
            $currency['symbol'] = $this->safe_currency_symbol(
                isset($currency['name']) ? $currency['name'] : '',
                isset($currency['symbol']) ? $currency['symbol'] : ''
            );
    ...
    private function safe_currency_symbol($currency_code, $symbol)
    {
        if (function_exists('get_safe_currency_symbol')) {
            return get_safe_currency_symbol($currency_code, $symbol);
        }
