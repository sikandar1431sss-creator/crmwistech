...
class Leads_model extends CRM_Model
{
    ...
    public function do_kanban_query($status, $search = '', $page = 1, $sort = [], $count = false)
    {
        ...
        if (!$has_permission_view) {
            $this->db->where('(assigned = ' . get_staff_user_id() . ' OR created_by=' . get_staff_user_id() . ' OR addedfrom=' . get_staff_user_id() . ' OR is_public=1)');
        }
        if ($search != '') {
    ...
    public function staff_can_access_lead($id, $staff_id = '')
    {
        ...
        if (total_rows('tblleads', 'id="' . $id . '" AND (assigned=' . $staff_id . ' OR is_public=1 OR created_by=' . $staff_id . ' OR addedfrom=' . $staff_id . ')') > 0) {
            return true;
        }
