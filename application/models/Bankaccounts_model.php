...
class Bankaccounts_model extends CRM_Model
{
    ...
    public function add($data)
    {
        ...
        if ($id) {
            logActivity('Bank Account Created [ID: ' . $id . ', Name: ' . $insert['bank_nick_name'] . ']');
        }
    ...
    public function update($id, $data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            logActivity('Bank Account Updated [ID: ' . $id . ', Name: ' . $update['bank_nick_name'] . ']');
            return true;
        }
    ...
    public function get_zoho_accounts($currency_code = '')
    {
    ...
    public function sync_zoho_accounts($zoho_accounts)
    {
        ...
        foreach ($zoho_accounts as $account) {
            ...
            if (isset($account['account_type']) && strtolower($account['account_type']) !== 'bank') {
                continue;
            }
            ...
            $code = !empty($existing['code'])
                ? $existing['code']
                : $this->build_zoho_bank_code($zoho_name, $account_id, $accounts);
            ...
            $accounts[] = [
                ...
                'account_type' => isset($account['account_type']) ? $account['account_type'] : '',
        ...
        logActivity('Zoho Bank Accounts Synced [Count: ' . count($accounts) . ']');
    ...
    private function build_zoho_bank_code($name, $account_id, $existing_accounts)
    {
        $base = strtolower(url_title($name, '_', true));
        $base = trim($base, '_');

        if ($base === '') {
            $base = 'bank_' . substr($account_id, -6);
        }
