...
class Invoices_model extends CRM_Model
{
    ...
    {
        parent::__construct();
        $this->ensure_bank_details_columns();
    }

    private function ensure_bank_details_columns()
    {
        if (!$this->db->table_exists('tblinvoices')) {
            return;
        }

        $fields = $this->db->list_fields('tblinvoices');

        if (!in_array('bank_account_id', $fields)) {
            $this->db->query("ALTER TABLE `tblinvoices` ADD `bank_account_id` INT(11) NOT NULL DEFAULT '0'");
        }

        if (!in_array('print_bank_details', $fields)) {
            $this->db->query("ALTER TABLE `tblinvoices` ADD `print_bank_details` TINYINT(1) NOT NULL DEFAULT '1'");
        }
    }
    ...
    public function get($id = '', $where = array())
    {
        ...
        $this->db->where($where);
       // $this->db->where('tblinvoices.id',1635);
        ...
        if (is_numeric($id)) {
            ...
            if ($invoice) {
                if (function_exists('get_safe_currency_symbol')) {
                    $invoice->symbol = get_safe_currency_symbol($invoice->currency_name, $invoice->symbol);
                }
        ...
        // $this->db->order_by('datecreated,number,YEAR(date)', 'desc');

        $invoices = $this->db->get()->result_array();

        foreach ($invoices as $index => $invoice) {
            if (function_exists('get_safe_currency_symbol') && isset($invoice['currency_name'], $invoice['symbol'])) {
                $invoices[$index]['symbol'] = get_safe_currency_symbol($invoice['currency_name'], $invoice['symbol']);
            }
        }
    ...
    public function get_invoice_items($id)
    {
        ...
        foreach ($items as $index => $item) {
            if (function_exists('get_safe_currency_symbol') && isset($item['currency_code'], $item['currency_symbol'])) {
                $items[$index]['currency_symbol'] = get_safe_currency_symbol($item['currency_code'], $item['currency_symbol']);
            }
        }
    ...
    public function mark_as_unpaid($id)
    {
        $this->db->where('id', $id);
        $this->db->update('tblinvoices', array(
            'status' => 1
        ...
        if ($this->db->affected_rows() > 0) {
            $this->log_invoice_activity($id, 'invoice_activity_marked_as_unpaid');
            do_action('invoice_marked_as_unpaid', $id);
            return true;
        }
        return false;
    }
    ...
    public function add($data, $expense = false)
    {
        // pre_array($data);
        $prevInvItems = array();
        if (isset($data['invoice_previous_items'])) {
            $prevInvItems = explode(',', $data['invoice_previous_items']);
        }

        $prevInvId = "";
        if (isset($data['prevInvoiceId'])) {
            $prevInvId = $data['prevInvoiceId'];
            unset($data['prevInvoiceId']);
        }
        ...
        if(!isset($data['number_format']) && empty($data['number_format'])){
        ...
        $SaveSendEmail = "";
        if (isset($data['SaveSend']) && isset($data['save_and_send_model'])) {
            $SaveSendEmail = $data['SaveSend'];
            $saveAndSend = true;
            unset($data['SaveSend']);
            unset($data['save_and_send_model']);
        }

        $saveAndAccept = false;
        if (isset($data['save_and_accept'])) {
            $saveAndAccept = true;
            unset($data['save_and_accept']);
        }
        ...
        $data['bank_account_id'] = isset($data['bank_account_id']) && $data['bank_account_id'] !== ''
            ? (int)$data['bank_account_id']
            : 0;
        $data['print_bank_details'] = isset($data['print_bank_details']) ? 1 : 0;
        ...
        if ($insert_id) {
            ...
            if (count($prevInvItems) > 0) {
                foreach ($prevInvItems as $pi) {
                    $this->db->where('id', $pi);
                    $this->db->update('tblitems_in', ['renewal_invoice_created' => '1']);
                }
            }

            if ($prevInvId) {
                $this->db->insert('tblinvoicesstatus', [
                    'invst_invoiceid' => $insert_id,
                    'invst_status' => 'open'
                ]);
            }

            if ($saveAndAccept) {
                $this->changeAcceptStatus($insert_id, 'accepted');
            }
            ...
            if (count($items) > 0) {
                foreach ($items as $key => $item) {
                    ...
                    if (isset($item['ItemRenwable'])) {
                        $renewable = ($item['ItemRenwable']);
                    }
            ...
            if ($saveAndSend === true) {
                ...
                $this->send_invoice_to_client($insert_id, '', true, $SaveSendEmail['cc'], true, $SaveSendEmail);
    ...
    public function update_total_tax($id)
    {
        ...
        $invoice = $this->get($id);
        if ($invoice->discount_type == 'fixed' && $invoice->discount_calculation == 'before_tax') {
            $discount_proportion = ($invoice->discount_percent / $invoice->subtotal) * 100;
        }
        foreach ($invoice->items as $item) {
            $item_taxes = get_invoice_item_taxes($item['id']);
            if ($invoice->discount_type == 'fixed' && $invoice->discount_calculation == 'before_tax') {
                $discount  = ($item['rate'] * $discount_proportion)/100;
                $item['rate'] = $item['rate'] - ($discount * $item['qty']);
            }
            if (count($item_taxes) > 0) {
                foreach ($item_taxes as $tax) {
                    ...
                    if ($tax_not_calc == true) {
                        ...
                        if ($invoice->discount_calculation == 'before_tax') {
                            array_push($taxes[$tax['taxname']]['total'], (($item['rate']) / 100 * $tax['taxrate']));
                        }else {
    ...
    public function copy($id)
    {
        ...
        $new_invoice_data['number'] = $this->make_next_invoice_num();
    ...
    public function update($data, $id)
    {
        ...
        if (isset($data['save_and_accept'])) {
            $saveAndAccept = true;
            update_invoice_status($id, true, true);
            unset($data['save_and_accept']);
        }
        ...
        if ($saveAndAccept) {
            $this->changeAcceptStatus($id, 'accepted');
        }

        if ($saveAndSend === true) {
            $this->send_invoice_to_client($id, '', true, $SaveSendEmail['cc'], true, $SaveSendEmail);
        }
     ...
     * @return boolean
     */
    public function send_invoice_to_client($id, $template = '', $attachpdf = true, $cc = '', $subject = null , $manually = false, $invoice_array = [])
    {
        $this->load->model('emails_model');
        ...
        if (isset($invoice_array['sent_to'])) {
            $sent_to = $invoice_array['sent_to'];
        }
        /* pre_array($sent_to); */
        if (is_array($invoice_array)) {
            $email_template_custom = $invoice_array['email_template_custom'];
        }

        $sendEmails = [];
        if (isset($invoice_array['emailto'])) {
            if (strpos($invoice_array['emailto'], ',') !== false) {
                $sendEmails = array_filter(explode(',', $invoice_array['emailto']));
            }
        }
        // pre_array($sendEmails);
        if (is_array($sendEmails) && count($sendEmails) > 0) {
            ...
            $i = 0;
            foreach ($sendEmails as $email_id) {
                if ($email_id != '') {
                    if ($attachpdf) {
                        $this->emails_model->add_attachment(array(
                            'attachment' => $attach,
                    ...
                    $contact = $this->clients_model->get_contact_by_email($email_id);
                    $merge_fields = array();
                    $merge_fields = array_merge($merge_fields, get_client_contact_merge_fields($invoice->clientid, $contact->userid));
                    ...
                    //if ($this->emails_model->send_email_template($template, $contact->email, $merge_fields, '', $cc, $email_template_custom, $subject)) {
     ...
     * @return mixed
     */
    public function get_invoices_renewals($data = null)
    {
        $salesPerson = 0;
        $status = '';
        $duration = '';
        ...
        $this->db->select('*');
        $this->db->select('tblitems_in.start_date as item_start_date');
        $this->db->select('tblitems_in.id as invoice_itmes_id');
        $this->db->select('tblitems_in.end_date as item_end_date');
        $this->db->select('tblitems_in.long_description as item_description');
        $this->db->select('item.description as item_name');
        $this->db->select('tblitems_in.start_date as from_date');
        $this->db->select('tblitems_in.rate as item_rate');
        $this->db->select('tblitems_groups.name as group_name');
        $this->db->from('tblitems_in');

        $this->db->where(['tblitems_in.is_renew_able' => '1', 'tblitems_in.rel_type' => 'invoice', 'tblitems_in.renewal_invoice_created' => '0']);
        ...
        if ($data <> null && count($data) > 0) {
            ...
            if (has_permission('renewals', '', 'view') || is_admin()) {
                $salesPerson = $data['assigned'];
            }

            $status = $data['status'];

            if ($data['in'] && $data['duration']) {

                if ($status == 'expired') {

                    // $time = '-' . $data['in'] . ' ' . $data['duration'];
                    // $this->db->where('tblitems_in.end_date >=', date("Y-m-d", strtotime("$today")));
                    $this->db->where('tblitems_in.end_date <=', $today);

                } else if($status == 'expiring') {
                    $time = '+' . $data['in'] . ' ' . $data['duration'];
                    $this->db->where('tblitems_in.end_date  <=', date("Y-m-d", strtotime("$today $time")));
                    $this->db->where('tblitems_in.end_date >=', $today);
                }else {
                    // $this->db->where('renewal_cancel', 1);
                }
            ...
            if($status == 'cancelled'){
                ...
                $this->db->where('renewal_cancel', 0);
            }

        } else {
            $this->db->where('tblitems_in.end_date <=', $next_30_days);
            $this->db->where('tblitems_in.end_date >=', $today);
            $this->db->where('renewal_cancel', 0);
            $salesPerson = $this->session->userdata['staff_user_id'];
        }

        $this->db->where('tblitems_in.renewal_invoice_created', '0');

        if (!is_admin()) {
            $salesPerson = $this->session->userdata['staff_user_id'];
        }

        $this->db->join('tblinvoices AS inv', 'tblitems_in.rel_id = inv.id', 'LEFT');
        $this->db->join('tblclients AS cl', 'inv.clientid = cl.userid', 'LEFT');
        $this->db->join('tblprojects AS proj', 'inv.project_id = proj.id', 'LEFT');
        $this->db->join('tblitems AS item', 'tblitems_in.description = item.description', 'LEFT');
        $this->db->join('tblitems_groups', 'tblitems_groups.id = item.group_id', 'LEFT');

        if ($salesPerson) {
            $this->db->join('tblcustomeradmins  AS cust_admin ', 'cust_admin.customer_id = inv.clientid', 'LEFT');
            //   $this->db->join('tblstaff  AS staff ', 'staff.staffid = cust_admin.staff_id', 'LEFT');
            $this->db->join('tblstaff  AS staff ', 'staff.staffid = inv.sale_agent', 'LEFT');
            $this->db->where('staff.staffid =', $salesPerson);
        } else {
            $this->db->join('tblcustomeradmins  AS cust_admin ', 'cust_admin.customer_id = inv.clientid', 'LEFT');
            $this->db->join('tblstaff  AS staff ', 'staff.staffid = inv.sale_agent', 'LEFT');
            // $this->db->join('tblcustomeradmins  AS cust_admin ', 'cust_admin.customer_id = cl.userid', 'LEFT');
            // $this->db->join('tblstaff  AS staff ', 'staff.staffid = cust_admin.staff_id', 'LEFT');
        }

        if($status == 'expired'){
            $this->db->order_by('tblitems_in.end_date', 'DESC');
        }else{
            $this->db->order_by('tblitems_in.end_date', 'ASC');
        }
        // $this->db->order_by('tblitems_in.end_date', 'ASC');
        $this->db->group_by('tblitems_in.id');
        return $this->db->get()->result();
    }

    public function invoice_item_renwal_cancel($data = null, $reason=null)
    {
        $renewal_user_id = get_staff_user_id();
        if ($data <> null && count($data) > 0) {
            foreach ($data as $key => $item_id){
                $this->db->where('id', $item_id);
                $this->db->update('tblitems_in', ['renewal_cancel' => 1,'renewal_user_id'=> $renewal_user_id,'renewal_cancel_reason'=> $reason]);
            }
        }
        return true;
    }
     ...
     * @return array
     */
    public function get_all_Customer_invoices($id, $currency = '')
    {
        $this->db->select('tblinvoices.*, tblcurrencies.name as currency_code, tblcurrencies.symbol as currency_symbol');
        $this->db->from('tblinvoices');
        $this->db->join('tblcurrencies', 'tblcurrencies.id = tblinvoices.currency', 'left');
     ...
     * @return mixed
     */
    public function get_all_Customer_proforma_invoices($id)
    {
        $this->db->select();
     ...
     * @return array
     */
    public function get_all_receipts_invoices($id)
    {
        $this->db->select('*');
     ...
     * @return mixed
     */
    public function changeAcceptStatus($id, $status)
    {
        $this->db->select('*');
        $this->db->from('tblinvoicesstatus');
        $this->db->where('invst_invoiceid', $id);
        $record = $this->db->get()->result();

        if (count($record) > 0 && $record <> null && !empty($record)) {
            $this->db->where('invst_invoiceid', $id);
            $save = $this->db->update('tblinvoicesstatus', ['invst_status' => $status]);

        } else {
            $save = $this->db->insert('tblinvoicesstatus', ['invst_invoiceid' => $id, 'invst_status' => $status]);
        }
        return $save;
    }
     ...
     * @return mixed
     */
    public function getInvoiceOwner($id)
    {
    ...
    public function set_reminder_sent_invoice_item($id, $rel_id)
    {
        $this->db->where(['id' => $id, 'rel_id' => $rel_id]);
        return $this->db->update('tblitems_in', ['first_reminder_sent' => '1']);
    }
     ...
     * @return mixed
     */
    public function create_auto_task_for_Invoice_accepted($invoice_id, $name)
    {

        $time = getDueDateForAutoTasks();
        $duration = $time['duedate_duration'];
        $duration_type = $time['duedate_duration_type'];

        $duedatetask = date('Y-m-d H:i', strtotime("+" . $duration . " " . $duration_type));
        $invoiceOwner = $this->getInvoiceOwner($invoice_id);

        $owner = 1;
        if (($invoiceOwner <> null)) {
            $owner = $invoiceOwner->sale_agent;
        }
        ...
        $data = [
            ...
            'reminder_duration_type' => $duration_type,
            'description' => "Invoice ",
            ...
            'visible_to_client' => 0,
            'milestone' => 0,
            'allow_duedate_change' => 0,
            'task_creation_type' => "auto",
            'assinged_to_user' => $owner,
            'is_notified' => 0
        ];
        // pre_array($data);
        $task_id = $this->tasks_model->add_auto_task($data);
        if ($task_id) {
            return $this->tasks_model->add_task_assignees(['taskid' => $task_id, 'assignee' => $owner]);
        }
    }

    public function getClientsInvoiceItems($client_id)
    {
        ...
        $this->db->select('tblitems_in.id as invoice_itmes_id');
        $this->db->select('tblitems_in.end_date as item_end_date');
        $this->db->select('tblitems_in.description as item_description');
        $this->db->select('item.description as item_name');
        $this->db->select('tblitems_in.start_date as from_date');
        $this->db->select('tblitems_in.rate as item_rate');
        $this->db->from('tblitems_in');
        ...
        $this->db->join('tblclients AS cl', 'inv.clientid = cl.userid', 'LEFT');
        $this->db->join('tblprojects AS proj', 'inv.project_id = proj.id', 'LEFT');
        $this->db->join('tblitems AS item', 'tblitems_in.service = item.id', 'LEFT');

        if ($salesPerson) {
            $this->db->join('tblcustomeradmins  AS cust_admin ', 'cust_admin.customer_id = ' . $salesPerson, 'LEFT');
        } else {
            $this->db->join('tblcustomeradmins  AS cust_admin ', 'cust_admin.customer_id = cl.userid', 'LEFT');
        }
        $this->db->where("rel_type", "invoice");
    ...
    public function receiptDiscountOnInvoice($id, $discount, $type = 'fixed')
    {
        $this->db->update('',[
    ...
    public function make_next_invoice_num()
    {

        $last_invoice_num = $this->get_last_invoice_number();

        if ($last_invoice_num <> null) {
            return $last_invoice_num->number + 1;
        }
        return false;
    }
}
