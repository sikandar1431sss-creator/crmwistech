...
class Proposals_model extends CRM_Model
{
    ...
    public function send_proposal_to_email_old($id, $template = '', $attachpdf = true, $cc = '', $subject)
    {
        $this->load->model('emails_model');
        ...
        $merge_fields = [];
        $merge_fields = array_merge($merge_fields, get_proposal_merge_fields($proposal->id));
        $sent = $this->emails_model->send_email_template($template, $proposal->email, $merge_fields, '', $cc, $subject);
    ...
    public function send_proposal_to_email($id, $template = '', $attachpdf = true, $cc = '', $subject, $manually = false)
    {
        ...
        if (!DEFINED('CRON') && $manually === false) {
            ...
            foreach ($contacts as $contact) {
                if (has_contact_permission('proposal', $contact['id'])) {
        ...
        if (is_array($sent_to) && count($sent_to) > 0) {
            ...
            foreach ($sent_to as $contact_id) {
                if ($contact_id != '') {
                    ...
                    $merge_fields = array();
                    $merge_fields = array_merge($merge_fields, get_client_contact_merge_fields($proposal->rel_id, $contact_id));
                    $merge_fields = array_merge($merge_fields, get_proposal_merge_fields($proposal->id));
