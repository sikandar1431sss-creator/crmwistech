...
class Reports extends Admin_controller
{
    ...
    public function balance_report()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                get_sql_select_client_company(),
                '(SELECT SUM(total) FROM tblinvoices WHERE tblinvoices.clientid = tblclients.userid AND status != 5 AND type="performa")',
                '(SELECT SUM(total) FROM tblinvoices WHERE tblinvoices.clientid = tblclients.userid AND status != 5 AND type="invoice")',
                '(SELECT SUM(total) FROM tblinvoices WHERE tblinvoices.clientid = tblclients.userid AND status != 5)',
                '(SELECT SUM(amount) FROM tblinvoicepaymentrecords WHERE tblinvoicepaymentrecords.client_id = tblclients.userid AND tblinvoicepaymentrecords.adjustment = 0)',
                '(SELECT SUM(amount) FROM tblinvoicepaymentrecords WHERE tblinvoicepaymentrecords.client_id = tblclients.userid AND tblinvoicepaymentrecords.adjustment = 1)',
                '(SELECT SUM(amount) FROM tblinvoicepaymentrecords WHERE tblinvoicepaymentrecords.client_id = tblclients.userid AND tblinvoicepaymentrecords.adjustment = 3)',
                '(SELECT SUM(total) FROM tblinvoices WHERE tblinvoices.clientid = tblclients.userid AND status != 5) - (SELECT SUM(amount) FROM tblinvoicepaymentrecords WHERE tblinvoicepaymentrecords.client_id = tblclients.userid)',
                //'(tblclients.userid - tblclients.userid)'
                //'(SELECT SUM(amount) FROM tblinvoicepaymentrecords WHERE tblinvoicepaymentrecords.client_id = tblclients.userid)',
                // '(SELECT SUM(amount) FROM tblinvoicepaymentrecords WHERE tblinvoicepaymentrecords.client_id = tblclients.userid) AS b',
            ];
            ...
            $sTable       = 'tblclients';
            $where        = [];
            array_push($where, 'AND ((SELECT SUM(total) FROM tblinvoices WHERE tblinvoices.clientid = tblclients.userid AND status != 5) - (SELECT SUM(amount) FROM tblinvoicepaymentrecords WHERE tblinvoicepaymentrecords.client_id = tblclients.userid)) > 0 AND tblclients.active = 1');
            $result = data_tables_init($aColumns, $sIndexColumn, $sTable, [], $where, [
                'userid',
            ]);
            $output  = $result['output'];
            ...
            foreach ($rResult as $aRow) {
                ...
                for ($i = 0; $i < count($aColumns); $i++) {
                    ...
                    if ($i == 0) {
                        ...
                        if($i == 1){
                            $footer_data['pinv'] += $_data;
                        }
                        if($i == 2){
                            $footer_data['inv'] += $_data;
                        }
                        if($i == 3){
                            $footer_data['total'] += $_data;
                        }
                        if($i == 4){
                            $footer_data['paid'] += $_data;
                        }
                        if($i == 5){
                            $footer_data['adjustment'] += $_data;
                        }
                        if($i == 6){
                            $footer_data['bad_debts'] += $_data;
                        }
                        if($i == 7){
                            $footer_data['remaining'] += $_data;
                        }
                        $_data = format_money($_data, $currency_symbol);
                    }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($proposalsTaxesSelect as $key => $tax) {
                array_splice($select, 8, 0, '(
                    ...
                    WHERE tblitems_in.rel_type="proposal" AND tblitemstax.taxname="' . $tax['taxname'] . '" AND tblitemstax.taxrate="' . $tax['taxrate'] . '" AND tblitems_in.rel_id=tblproposals.id) as total_tax_single_' . $key);
            }
    ...
    public function invoices_report()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $client = get_customer_city($aRow['userid']);

                $row[] = $aRow['year'];

                $row[] = get_country_name($client->country);
