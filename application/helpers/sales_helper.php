...
if (!function_exists('format_customer_info')) {
    ...
    function format_customer_info($data, $for, $type, $companyLink = false)
    {
        ...
        if ($type == 'billing') {
            ...
            $country = get_country($data->client->country);
        } elseif ($type == 'shipping') {
...
if (!function_exists('get_table_items_and_taxes')) {
    ...
    function get_table_items_and_taxes($items, $type, $admin_preview = false)
    {
        ...
        foreach ($items as $item) {
            ...
            $itemHTML        = '<style>ul br, ol br, span br, p br {display:none;}</style>';
            $trAttributes    = '';
            $tdFirstSortable = '';
            ...
            if (!empty($item['long_description'])) {
                ...
                if($item["is_renew_able"]){
                    $itemHTML .= '<br/><span style="color:#424242;"> Starting From ' . date_format_dmy($item["start_date"]) . ' to ' . date_format_dmy($item["end_date"]).'</span>';
                }
            }
...
if (!function_exists('get_table_items_and_taxes_pdf')) {
    /**
     * Function for all table items HTML and PDF
     * @param  array  $items         all items
     * @param  [type]  $type          where do items come form, eq invoice,estimate,proposal etc..
     ...
     * @return array
     */
    function get_table_items_and_taxes_pdf($items, $type, $admin_preview = false)
    {
        ...
        foreach ($items as $ky => $item) {
            ...
            if (get_option('show_tax_per_item') == 1) {
              ...
              /*  if($hook_data['item_taxes'][0]['taxname'] == 'Zero RAted|0.00'){
                    $total_tax1 =  $result['taxes'][$hook_data['item_taxes'][0]['taxname']]['total'][$vat_coounter_0];

                }else {
                    $total_tax1 =  $result['taxes'][$hook_data['item_taxes'][0]['taxname']]['total'][$vat_coounter_5];

                }
                if ($rel_data) {
                    if ($rel_data->discount_percent != 0 && $rel_data->discount_type == 'before_tax') {
                        $total_tax_tax_calculated = ($total_tax1 * $rel_data->discount_percent) / 100;
                        $total_tax = ($total_tax1 - $total_tax_tax_calculated);
                    } elseif ($rel_data->discount_total != 0 && $rel_data->discount_type == 'before_tax') {
                        $t = ($rel_data->discount_total / $rel_data->subtotal) * 100;
                        $total_tax1 = ($total_tax1 - $total_tax1 * $t / 100);
                    }
                }*/


              //  $itemHTML .= '<td align="right">' ._format_number($total_tax1) . '</td>';
            }
            $total_tax_pdf =  $result['taxes']['VAT|5.00']['total'][$ky];

            $itemHTML .= '<td align="right">' ._format_number($total_tax_pdf) . '</td>';
           ...
           /* if($hook_data['item_taxes'][0]['taxname'] == 'Zero RAted|0.00'){
                $vat_coounter_0++;
            }else if($hook_data['item_taxes'][0]['taxname'] == 'VAT|5.00') {
                $vat_coounter_5++;
            }*/
