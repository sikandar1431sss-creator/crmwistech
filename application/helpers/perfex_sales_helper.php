...
function get_invoice_total_left_to_pay($id, $invoice_total)
{
    ...
    foreach ($payments as $payment) {
        $invoice_total -= ($payment['amount'] + $payment['discount']);
    }
...
function format_estimate_numberpdf($id)
{
    $CI =& get_instance();
    $CI->db->select('date,number,prefix,number_format')->from('tblestimates')->where('id', $id);
    $estimate = $CI->db->get()->row();
...
function invoice_pdf($invoice, $tag = '')
{
    ...
    // In case user want to include {invoice_number} in PDF offline mode description
    foreach ($payment_modes as $key => $mode) {
        if (isset($modepdf_email_signature['description'])) {
...
function estimate_pdf($invoice, $tag = '')
{
    $CI =& get_instance();
    load_pdf_language($invoice->clientid);
    $CI->load->library('pdf');
    $invoice_number = format_estimate_numberpdf($invoice->id);
 ...
 * @return mixed object
 */
function estimate_pdfold($estimate, $tag = '')
{
    $CI =& get_instance();
...
if (!function_exists('format_customer_info')) {
    ...
    function format_customer_info($data, $for, $type, $companyLink = false)
    {
        ...
        if ($for == 'invoice' || $for == 'estimate' || $for == 'payment' || $for == 'receipt') {
