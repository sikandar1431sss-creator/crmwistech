...
function tasks_rel_name_select_query()
{
    return '(CASE rel_type
        ...
        WHEN "lead" THEN (SELECT CASE tblleads.email WHEN "" THEN tblleads.name ELSE CONCAT(tblleads.name ) END FROM tblleads WHERE tblleads.id=tblstafftasks.rel_id)
...
function tasks_rel_description_select_query()
{
    return '(CASE rel_type
        WHEN "lead" THEN (SELECT CASE tblleads.email WHEN "" THEN tblleads.description ELSE CONCAT(tblleads.description ) END FROM tblleads WHERE tblleads.id=tblstafftasks.rel_id)
        ELSE NULL
        END)';
}

function tasks_rel_contact_select_query()
{
    return '(CASE rel_type
        WHEN "lead" THEN (SELECT CASE tblleads.email WHEN "" THEN tblleads.email ELSE CONCAT(tblleads.email ) END FROM tblleads WHERE tblleads.id=tblstafftasks.rel_id)
        ELSE NULL
        END)';
}
...
function get_invoice_status($id)
{
    $CI =& get_instance();
    $CI->db->select('*');
    $CI->db->from('tblinvoicesstatus');
    $CI->db->where(['invst_invoiceid' => $id]);
    $items = $CI->db->get()->row();

    return $items;
}

function getLeadReminderTime(){
    $data = array();
    $data['reminder_duration']          = get_option('lead_assigned_task_duration');
    $data['reminder_duration_type']     = get_option('lead_assigned_task_duration_type');

    return $data;
}

function getDueDateForAutoTasks(){
    $data = array();
    $data['duedate_duration']          = get_option('lead_assigned_task_duedate_duration');
    $data['duedate_duration_type']     = get_option('lead_assigned_task_duedate_duration_type');

    return $data;
}

function getFirstRenewalReminders(){
    $data = array();
    $data['reminder_duration']          = get_option('renewal_first_reminder_duration');
    $data['reminder_duration_type']     = get_option('renewal_first_reminder_duration_type');

    return $data;
}

function getSecondRenewalReminders(){
    $data = array();
    $data['reminder_duration']          = get_option('renewal_second_reminder_duration');
    $data['reminder_duration_type']     = get_option('renewal_second_reminder_duration_type');

    return $data;
}

function getThirdRenewalReminders(){
    $data = array();
    $data['reminder_duration']          = get_option('renewal_third_reminder_duration');
    $data['reminder_duration_type']     = get_option('renewal_third_reminder_duration_type');

    return $data;
}


function getReceiptNumById($id){

    $CI =& get_instance();
    $CI->db->select('receipt_num');
    $CI->db->where('receipt_id', $id);
    $CI->db->from('tblreciepts');
    $receipt_num = $CI->db->get()->row()->receipt_num;

    return $receipt_num;
}

/**
 * Get the client company information against the invoice number
 */
function getClientCompanyNameWithInvoice($invoice_number){
    $CI = &get_instance();
    $CI->db->select('company');
    $CI->db->join('tblclients', 'tblclients.userid=tblinvoices.clientid');
    $CI->db->where('tblinvoices.id', $invoice_number);
    return $CI->db->get('tblinvoices')->row()->company;
}

/**
 * Get the Latest comment action date
 */
function getLastCommentIdOfEachTask(){
    $CI = &get_instance();
    $CI->db->select('MAX(id) AS id');
    $CI->db->group_by('taskid');
    $result = $CI->db->get('tblstafftaskcomments')->result_array();
