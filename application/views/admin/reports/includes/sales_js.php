<script>
 ...
 var report_balance = $('#balance-report');
 ...
 $(function() {
   ...
   $('.table-proposals-report').on('draw.dt', function() {
     ...
     $('.table-balance-report').on('draw.dt', function() {
         var balanceReportTable = $(this).DataTable();
         var sums = balanceReportTable.ajax.json().sums;
         $(this).find('tfoot').addClass('bold');
         $(this).find('tfoot td').eq(0).html("<?php echo _l('invoice_total'); ?> (<?php echo _l('per_page'); ?>)");
         $(this).find('tfoot td.pinv').html(sums.pinv);
         $(this).find('tfoot td.inv').html(sums.inv);
         $(this).find('tfoot td.total').html(sums.total);
         $(this).find('tfoot td.paid').html(sums.paid);
         $(this).find('tfoot td.adjustment').html(sums.adjustment);
         $(this).find('tfoot td.bad_debts').html(sums.bad_debts);
         $(this).find('tfoot td.remaining').html(sums.remaining);
   ...
   // Clear custom date picker
       ...
       if (type == 'total-income') {
         ...
         report_customers.removeClass('hide');
           ...
           report_balance.removeClass('hide');
 ...
 function balance_report() {
     if ($.fn.DataTable.isDataTable('.table-balance-report')) {
         $('.table-balance-report').DataTable().destroy();
     }
     initDataTable('.table-balance-report', admin_url + 'reports/balance_report', false, false, fnServerParams, [0, 'asc']);
 }
 ...
 function items_report(){
   ...
   function gen_reports() {
     ...
     if (!$('.chart-income').hasClass('hide')) {
       ...
       customers_report();
     }else if (!report_balance.hasClass('hide')) {
         balance_report();
     }
