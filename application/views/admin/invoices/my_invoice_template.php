...
if ($this->input->get('renew-items')) {
    $items = $this->input->get('renew-items');
    $prevInvoiceItemsIn = $items;
    $invoiceItemsNew = $this->invoices_model->get_all_items(explode(',', $items));
}

if ($this->input->get('prev-invoice')) {
    $prevInv = $this->input->get('prev-invoice');
}

if (isset($prevInvoiceItemsIn)) {
    echo form_hidden('invoice_previous_items', $prevInvoiceItemsIn);
}

if (isset($prevInv)) {
    ?>
    <input type="hidden" name="prevInvoiceId" value="<?= $prevInv; ?>"/>
    <?php
}
...
$prevDate = "";
if ((isset($invoice))) {
    $prevType = $invoice->type;
}

if ((isset($invoice))) {
    $prevDate = $invoice->date;
}
?>

<span class="hidden" id="prevInvoiceType"><?= $prevType; ?></span>
<span class="hidden" id="PrevInvDate"><?= $prevDate; ?></span>
    ...
    <div class="panel-body">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                $next_invoice_number = make_next_invoice_num();
                ...
                if (isset($invoice)) {
                    ...
                    if ($invoice->type == "performa") {
                        $next_invoice_number = make_next_performa_invoice_number();
                    ...
                    if ($this->input->get('type') == 'invoice') {
                        $next_invoice_number = make_next_invoice_num();
                        $prefix = get_option('invoice_prefix');
                        $inv_prefix = $prefix;
                    }
                ...
                <div class="form-group">
                  ...
                  <?php if (isset($invoice)) {
                      $settings_sales_invoice_prefix = _l("settings_sales_invoice_prefix");
                      $admin_url = admin_url('invoices/update_number_settings/' . $invoice->id);
                      ?>
                      <a href="#" onclick="return false;" data-toggle="popover" data-container='._transaction_form'
                         data-html="true"
                         data-content='<label class="control-label"><?php echo $settings_sales_invoice_prefix; ?></label><div class="input-group"><input name="s_prefix" type="text" id="wis_inv_prefix"  class="form-control" value="<?php echo $prefix; ?>" ></div><button type="button" onclick="save_sales_number_settings(this); return false;" data-url="<?php echo $admin_url; ?>" class="btn btn-info btn-block mtop15"><?php echo _l("submit"); ?></button>'>
                    <i class="hidden fa fa-cog"></i>
                    </a>
                  <?php } ?>
                  <span id="inv_prefix_tax"> <?php echo trim($prefix);?></span>
                  </span>
                        <input type="text" name="number" id="wis_inv_number" class="form-control"
                               value="<?php echo $_invoice_number; ?>"
                               data-isedit="<?php echo $isedit; ?>"
                               data-origINV-inal-number="<?php echo $data_original_number; ?>">
                        <?php if ($format == 3) { ?>
                            <span class="input-group-addon">
                     ...
                     <span id="prefix_year" class="format-mm-yyyy"><?php echo $yyyy; ?></span>
                        ...
                        <?php } ?>
                        <input type="hidden" name="prefix" id="wis_inv_number_prefix" value='<?= $inv_prefix; ?>'>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6">
            ...
            $invoice_type = 'performa';

            if ((isset($invoice) && $invoice->type == 'invoice')) {
            ...
            <div class="col-md-6">
                <div class="panel_s no-shadow">
                    <div class="form-group">
                        <label for="tags" class="control-label"> <?php echo _l('invoice_type'); ?></label>
                        <select name="type" id="wis_invoice_type" class="selectpicker"
                                data-width="100%"
                                data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                            <option value="performa" <?php if ($invoice_type == 'performa') {
                                echo 'selected';
                            } ?>><?php echo _l('invoice_type_performa'); ?></option>
                            <option value="invoice" <?php if ($invoice_type == 'invoice') {
                                echo 'selected';
                            } ?>><?php echo _l('invoice_type_tax'); ?></option>
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group mbot15 select-placeholder">
                        ...
                        <select class="selectpicker" name="bank_account_id" id="bank_account_id"
                                data-live-search="true" data-width="100%"
                                data-none-selected-text="Select bank account">
                            <option value="">Select bank account</option>
                            <?php foreach ((isset($bank_accounts) ? $bank_accounts : []) as $bank_account) {
                                $selected = '';
                                if (isset($invoice) && isset($invoice->bank_account_id) && (int)$invoice->bank_account_id === (int)$bank_account['id']) {
                                    $selected = ' selected';
                                }
                                $bank_label = $bank_account['bank_nick_name'];
                                if ($bank_label === '') {
                                    $bank_label = $bank_account['full_bank_name'];
                                }
                                ?>
                                <option value="<?php echo html_escape($bank_account['id']); ?>"
                                        data-currency-id="<?php echo html_escape($bank_account['currency_id']); ?>"
                    ...
                    <div class="checkbox checkbox-primary">
                        <input type="checkbox"
                               ...
                               value="1" <?php if (!isset($invoice) || !isset($invoice->print_bank_details) || (int)$invoice->print_bank_details === 1) { echo 'checked'; } ?>>
        ...
        <div class="table-responsive s_table">
            <table class="table invoice-items-table items table-main-invoice-edit no-mtop">
                ...
                <tr>
                    ...
                    ?>
                    <th width="" class="text-left" style="text-align: center;"><?php echo _l('Renewable'); ?></th>
                    <th width="" class="text-left" style="text-align: center;"><?php echo _l('From'); ?></th>
                    <th width="" class="text-left" style="text-align: center;"><?php echo _l('To'); ?></th>
                ...
                <tr class="main">
                    ...
                    <td>
                        <textarea name="long_description" rows="4"
                                  id="long_description_"
                                  class="form-control item_long_description"
                                  placeholder="<?php echo _l('item_long_description_placeholder'); ?>"></textarea>
                        <input type="hidden" value="other" id="input_item_type_dom"/>
                    ...
                    <td>
                        <div class="form-group">
                            <div class="checkbox checkbox-primary no-mtop checkbox-inline">
                                <input type="checkbox" id="isRenewable-0" class="isRenewable" name="isRenewable"
                                       value="0">
                                <label for="isRenewable-0">Yes</label>
                            </div>
                        </div>
                    </td>
                    <td>
                        <div class="form-group">
                            <div class="input-group date isRenewable-0 hidden">
                                <input type="text" id="start_date-0" name="start_date"
                                       class="form-control from_to_date datepicker" value="<?= date('Y-m-d'); ?>"
                                       aria-invalid="false">
                                <div class="input-group-addon">
                    ...
                    <td>
                        <div class="form-group">
                            <div class="input-group date isRenewable-0 hidden">
                                <input type="text" id="end_date-0" name="end_date"
                                       class="form-control from_to_date datepicker" value="<?= date('Y-m-d'); ?>"
                                       aria-invalid="false">
                                <div class="input-group-addon">
                                    <i class="fa fa-calendar calendar-icon"></i>
                ...
                <?php if (isset($invoice) || isset($add_items) || isset($invoiceItemsNew)) {
                    $i = 1;
                    ...
                    if (isset($invoiceItemsNew)) {
                        $add_items = $invoiceItemsNew;
                        $items_indicator = 'newitems';
                    }
                    foreach ($add_items as $item) {
                        ...
                        $table_row .= '<input type="hidden" value="' . $item_type . '" id="item_type_dom' . $i . '" name="' . $items_indicator . '[' . $i . '][item_type]">';
                        $table_row .= '</td>';
                        ...
                        // If item type is domain then check url

                        if ($item_type == "domain") {
                            $table_row .= '<td><input type="url" onblur="checkTypeURL(this)"  name="' . $items_indicator . '[' . $i . '][long_description]" class="form-control item_long_description_url"   id="long_description' . $i . '"  value="' . clear_textarea_breaks(strip_tags($item['long_description'])) . '" /></td>';
                        } else {
                            $table_row .= '<td><textarea name="' . $items_indicator . '[' . $i . '][long_description]" class="form-control item_long_description" id="long_description' . $i . '"  rows="5">' . clear_textarea_breaks($item['long_description']) . '</textarea></td>';
                        }
                        ...
                        if ($item['is_renew_able'] || $item_type == "domain") {
                            $isChecked = 'checked';
                            $hidden = '';
                            $required = 'required';
                        }
                        ...
                        $table_row .= '             <input type="checkbox" class="isRenewable"  ' . $isChecked . ' id="isRenewable-' . $i . '"  name="' . $items_indicator . '[' . $i . '][ItemRenwable]" value="1">';
                        $table_row .= '         <label for="isRenewable-' . $i . '" >Yes</label></div></div>';
                        $table_row .= '    </td>';

                        $table_row .= '    <td>';
                        $table_row .= '        <div class="form-group">';
                        $table_row .= '            <div class="input-group date ' . $hidden . ' isRenewable-' . $i . '">';
                        $table_row .= '                 <input type="text" ' . $required . ' id="start_date-' . $i . '"  name="' . $items_indicator . '[' . $i . '][start_date]" class="form-control from_to_date datepicker" value="' . date("d-m-Y", strtotime($item['start_date'])) . '" aria-invalid="false">';
                        ...
                        $table_row .= '        <div class="form-group">';
                        $table_row .= '            <div class="input-group date ' . $hidden . ' isRenewable-' . $i . '">';
                        $table_row .= '                 <input type="text" ' . $required . ' id="end_date-' . $i . '"  name="' . $items_indicator . '[' . $i . '][end_date]" class="form-control from_to_date datepicker" value="' . date("d-m-Y", strtotime($item['end_date'])) . '" aria-invalid="false">';
                        $table_row .= '                 <div class="input-group-addon"><i class="fa fa-calendar calendar-icon"></i></div>';
                        $table_row .= '            </div>';
                        $table_row .= '        </div>';
                        $table_row .= '    </td>';
            ...
            <div class="panel-body bottom-transaction">
                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        <label><?= _l('terms_and_conditions'); ?></label>
                        <select name="noteType" class="form-control" id="selectInvoiceNotes">
                            <option value=""><?= _l('terms_and_conditions'); ?></option>
                            <?php
                            $custom_notes = getByTypePosition("invoice");
                            if (count($custom_notes) > 0) {
                                foreach ($custom_notes as $note) {
                                    ?>
                                    <option value="<?= $note->cn_id; ?>"><?= $note->cn_title; ?></option>
                                    <?php
                                }
                            }
                            ?>
                ...
                <div class="form-group" id="customNoteDiv">
                    <textarea id="terms" name="terms" class="form-control tinymce terms"><?= $value; ?></textarea>
                </div>
                <hr/>
...
<!-- ####################### Modal box for description edit -->
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <?php echo render_textarea('description', '', '', array('id' => 'set_description'), array(), '', 'tinymce'); ?>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" id="apply_description" data-dismiss="modal">
                    Apply
                </button>
...
<script>
    $(function () {
        function applySelectedBankCurrency() {
            var $selectedBank = $('#bank_account_id option:selected');
            var currencyId = $selectedBank.data('currency-id');
            var currencyCode = ($selectedBank.data('currency-code') || '').toString().toUpperCase();
            ...
            if (!currencyId && currencyCode !== '') {
                $currency.find('option').each(function () {
                    ...
                    var optionSubtext = ($(this).data('subtext') || '').toString().toUpperCase();

                    if (optionText === currencyCode || optionSubtext === currencyCode) {
                        currencyId = $(this).val();
                        return false;
                    }
                });
        ...
        $('body').on('changed.bs.select change', '#bank_account_id', function () {
            applySelectedBankCurrency();
        });

        $('body').on('submit', 'form._transaction_form', function () {
            applySelectedBankCurrency();
        });
    });
</script>
...
<style>
    ...
    .custom_class_txt_area {
        width: 326px;
        height: 82px !important;
    }

    .input-group.date .from_to_date {
