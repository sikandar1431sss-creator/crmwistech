...
if (has_permission('leads', '', 'view') && $this->ci->input->post('view_created')) {
    array_push($where, 'AND addedfrom =' . $this->ci->input->post('view_created'));
}
...
foreach ($rResult as $aRow) {
    ...
    if ($aRow['addedfrom'] != 0) {
        $full_name = get_staff_full_name($aRow['addedfrom']);
