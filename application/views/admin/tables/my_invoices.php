  ...
  if ($this->ci->input->post('posted')) {
      array_push($filter, 'AND trim(tblinvoices.zoho_id) !="" ');
  }
  if ($this->ci->input->post('not_posted')) {
      array_push($filter, 'AND trim(tblinvoices.zoho_id) ="" AND tblinvoices.type="invoice"');
  }
  ...
  if (!has_permission('invoices', '', 'view')) {
      if(($clientid == 1617 || $clientid == 1098 || $clientid == 310 || $clientid == 1606 || $clientid == 111 || $clientid == 1525 || $clientid == 1295 || $clientid == 1355 || $clientid == 1800 || $clientid == 1357 || $clientid == 1380 || $clientid == 327 || $clientid == 210 || $clientid == 1135 || $clientid == 146 || $clientid == 1815 || $clientid == 78 || $clientid == 1126 || $clientid == 1143 || $clientid == 166 || $clientid == 1358 || $clientid == 211) && get_staff_user_id() == 36){

      }else {
      //    echo $project_id.'<br>'.get_staff_user_id();
        //  die('here');
  ...
  array_multisort(array_column($rResult, 'id'), SORT_DESC, $rResult);

  foreach ($rResult as $aRow) {
      $CI     = & get_instance();
  //die;
      $zoho_data = $CI->db->query('SELECT `zoho_id`,`type` FROM tblinvoices WHERE id='.$aRow['id']);
      $zoho_data_result = $zoho_data->result_array();
      $row = [];

      $numberOutput = '';

      $invNum = format_invoice_number($aRow['id']);

      $isPerf = explode('-',$invNum);

      // If is from client area table
      if (isset($isPerf[0]) && $isPerf[0] == 'PINV') {

          if (is_numeric($clientid) || $project_id) {
              $numberOutput = '<a style="color:#cc6600 !important;" href="' . admin_url('invoices/list_invoices/' . $aRow['id']) . '" target="_blank">' . $invNum . '</a>';
          } else {
              $numberOutput = '<a style="color:#cc6600 !important;" href="' . admin_url('invoices/list_invoices/' . $aRow['id']) . '" onclick="init_invoice(' . $aRow['id'] . '); return false;">' . $invNum . '</a>';
          }
      ...
      $currencySymbol = $aRow['symbol'];
      if (function_exists('get_safe_currency_symbol')) {
          $currencySymbol = get_safe_currency_symbol($aRow['currency_name'], $currencySymbol);
      }

      $row[] = format_money($aRow['total'], $currencySymbol);

      $row[] = format_money($aRow['total_tax'], $currencySymbol);
      ...
      $zoho_status='';
      if ($zoho_data_result[0]['type'] == "invoice" && $aRow['tblinvoices.status'] != 5) {
