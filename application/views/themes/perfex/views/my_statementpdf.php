  ...
  <td width="27%">';
    ...
    if (isset($data['invoice_id'])) {
        ...
        if ($data['invoice_status'] != 5) {
            $tblhtml .= '<a style="color:' . $color . '" href="' . admin_url('invoices/list_invoices/' . $data['invoice_id']) . '" target="_blank">' . format_invoice_number($data['invoice_id']) . '</a> ' . $data['adminnote'];
        } else {
            $tblhtml .= '<strike>' . '<a style="color:' . $color . '" href="' . admin_url('invoices/list_invoices/' . $data['invoice_id']) . '" target="_blank">' . format_invoice_number($data['invoice_id']) . '</a></strike>' . $data['adminnote'];
        }
