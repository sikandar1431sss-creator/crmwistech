...
if ($invoice->status == 6) {
    if ($invoice->type == "invoice") {
        $info_right_column .= '<span style="font-weight:bold;font-size:27px;">' . _l('invoice_pdf_heading_perform') . '</span><br />';
    } else {
        $info_right_column .= '<span style="font-weight:bold;font-size:27px;">' . _l('invoice_pdf_heading_perform') . '</span><br />';
    }
} else {
...
$info_right_column .= '<b># ' . $invoice_number . '</b>';
if($invoice->pinv_reference != '') {
    $info_right_column .= '<br><b>Ref: PINV-00' . $invoice->pinv_reference . '</b>';
}
...
$info_right_column .= '<br/>' . _l('invoice_date') . ': ' . date_format_dmy($invoice->date) . '</1b>';
if (!empty($vat)) {
    $info_right_column .= '<br/>' . _l('vat_number') . ': ' . $vat . '</1b>';
}
$info_right_column .= '<br/> 12A-04, Damac Smart Heights,</br>';
$info_right_column .= '<br/> TECOM, Dubai,</br>';
$info_right_column .= '<br/> United Arab Emirates</br>';
...
$info_left_column .= format_customer_info($invoice, 'invoice', 'billing');
$info_left_column .= '</div><br>';
$pdf->MultiCell(($dimensions['wk'] / 2) - $dimensions['lm'], 30, $info_left_column, 0, 'J', 0, 0, '', '', true, 0, true, true, 0);
// write the second column
$pdf->MultiCell(($dimensions['wk'] / 2) - $dimensions['rm'], 30, $info_right_column, 0, 'R', 0, 1, '', '', true, 0, true, false, 0);
$pdf->ln(6);
...
$tblhtml = '
<table width="100%" bgcolor="#fff" border="1" cellspacing="0" cellpadding="8" style="margin-top: 50px; clear: both;">
...
$tblhtml .= '<th width="10%" align="right">' . _l('invoice_table_tax_heading') . ' 5%</th>';
if (get_option('show_tax_per_item') == 1) {
    $tblhtml .= '<th width="15%" align="right">' . _l('invoice_table_tax_heading') . '</th>';
}

$tblhtml .= '<th width="10%" align="right">' . _l('invoice_table_amount_heading') . '</th>
</tr>';

// Items
$tblhtml .= '<tbody>';

$items_data = get_table_items_and_taxes_pdf($invoice->items, 'invoice');
...
$tbltotal .= '<table cellpadding="6" style="font-size:' . ($font_size + 4) . 'px"  border="1">';
$tbltotal .= '
<tr>
...
foreach ($taxes as $tax) {
    ...
    if ($invoice->discount_type == 'percentage') {
        if ($invoice->discount_percent != 0 && $invoice->discount_calculation == 'before_tax') {
    ...
    if ($invoice->discount_percent != 0 && $invoice->discount_type == 'before_tax') {
        $total_tax_calculated = ($total * $invoice->discount_percent) / 100;
        $total                = ($total - $total_tax_calculated);
    } elseif ($invoice->discount_total != 0 && $invoice->discount_type == 'before_tax') {
...
if (count($invoice->payments) > 0 && get_option('show_transactions_on_invoice_pdf') == 1) {
    ...
    $pdf->SetFont($font_name, '', $font_size);
    $pdf->Ln(4);
    $tblhtml = '<table width="100%" bgcolor="#fff"  border="1" cellspacing="0" cellpadding="5" border="0">
...
$bank_details_html = get_invoice_bank_details_html($invoice);
if (!empty($invoice->terms) || $bank_details_html !== '') {
    ...
    if ($bank_details_html !== '') {
        $pdf->Ln(2);
        $pdf->writeHTMLCell('', '', '', '', $bank_details_html, 0, 1, false, true, 'L', true);
    }
}
 

// $staff = get_staff_info_signature($invoice->sale_agent);
...
$css = file_get_contents(FCPATH . 'assets/css/invoices.css');
